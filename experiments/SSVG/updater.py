...
class SSVGUpdater(chainer.training.StandardUpdater):
    ...
    def enc_loss(self, enc, x1, x2, x3, mu1, mu3, ln_var1, ln_var3, x1_rec, x2_rec, x3_rec, y1_fake, y2_fake, y3_fake):
        ...
        adv_loss = 0
        # rec loss
        rec_loss += F.bernoulli_nll(x1, x1_rec) / 3.0 / batchsize
        rec_loss += F.bernoulli_nll(x3, x3_rec) / 3.0 / batchsize
        # kl_loss
        kl_loss += F.gaussian_kl_divergence(mu1, ln_var1) / 3.0 / batchsize
        kl_loss += F.gaussian_kl_divergence(mu3, ln_var3) / 3.0 / batchsize
        # seq loss
        seq_loss += F.bernoulli_nll(x2, x2_rec) / 3.0 / batchsize
        # adv_loss
